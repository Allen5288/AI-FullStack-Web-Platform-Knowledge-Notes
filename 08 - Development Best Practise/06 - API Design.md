...
- It must be pragmatic:
  ...
  - It should perform well. None of these design practices should introduce a huge performance overhead. If we see this happening, then we need to re-visit that particular design practice. We recommend ["Microservices from Day One"](https://www.amazon.com/Microservices-Day-One-scalable-software-ebook/dp/B01MSXJ10K/ref=dp_kinw_strp_1) for specifics on how to monitor services.
...
The [URI (Uniform Resource Indicator)](http://searchsoa.techtarget.com/definition/URI) / Path is the path to a resource exposed by an API. Here are the key principles:
...
This is a very simple implementation of HAL (JSON Hypermedia API Language). For further information, please visit the [HAL site](http://stateless.co/hal_specification.html).
...
Given the example results in [Leading the Consumer](https://github.com/australiaitgroup/Guide-to-JS-Project/blob/master/api-design.md#pagination---leading-the-consumer), the client view might generate a links in its view like this ERB template example:
...
- Filter/sort/search on fields that are sortable field/column based on the most optimal way to access the resource's data (mot likely on a sortable/indexable column in the underlying database). Please see [Pagination](https://github.com/australiaitgroup/Guide-to-JS-Project/blob/master/api-design.md#pagination) for further details.
...
- [Cross-Origin Resource Sharing (CORS)](http://en.wikipedia.org/wiki/Cross-origin_resource_sharing) - CORS is recommended over JSONP because CORS is a [W3C standard](http://www.w3.org/TR/cors/) and supported by all modern browsers.
...
[HTTP Status Codes](https://github.com/australiaitgroup/Guide-to-JS-Project/blob/master/api-design.md#http-status-codes) are sufficient for expressing statuses and error conditions for APIs that are consumed by other APIs. Internal APIs could handle errors (especially uncaught exceptions) and return the information in the JSON response as follows:
...
Based on the [JSON API Error Formatting Guidelines](http://jsonapi.org/format/#errors), APIs used by consumer-facing applications **could** provide the following fields in the error response:

- Required:
  - `code` - An application-specific error code, expressed as a string value.
- Optional:
  - `title` - A short, human-readable summary of the problem. It **should not** change from occurrence to occurrence of the problem. This would be non-localized (i.e., English) because developers are the audience this.
...
Of the several Hypermedia specifications we reviewed (see [Hypermedia Specifications Considered](https://github.com/australiaitgroup/Guide-to-JS-Project/blob/master/api-design.md#hypermedia-specifications-considered)), we prefer [HAL](https://tools.ietf.org/html/draft-kelly-json-hal-06) because:
...
- The root object **must** be a Resource Object, which is one of:
  ...
  - `_embedded` - Optional. Contains embedded resources, which could be a full or partial version of the representation served from the target URI.
...
- [Pagination](https://github.com/australiaitgroup/Guide-to-JS-Project/blob/master/api-design.md#pagination---leading-the-consumer---proposed)
...
All of our APIs should use [Swagger](http://swagger.io/) to generate documentation. Many of our APIs already use it successfully. With Swagger, you use a JSON to specify an API's endpoints, and then generate the documentation as HTML.
...
- Reduce the Response Size
  - Use [Pagination](https://github.com/australiaitgroup/Guide-to-JS-Project/blob/master/api-design.md#pagination) to manage large result sets.
  - [Condense the JSON response with HTTP Compression](https://github.com/australiaitgroup/Guide-to-JS-Project/blob/master/api-design.md#condense-json-response-with-http-compression).
- Make the API work less with a [Conditional `GET` with Caching and `ETag`](https://github.com/australiaitgroup/Guide-to-JS-Project/blob/master/api-design.md#conditional-get-with-caching-and-etag).
...
| `Last-Modified` | The timestamp that the resource itself was modified last (in [RFC 1123 format](http://www.csgnetwork.com/timerfc1123calc.html)). | `Last-Modified: Sun, 06 Nov 1994 08:49:37 GMT` |
...
- [VCR](https://github.com/vcr/vcr) to record and replay API calls so that test suites run faster. See [How to Use VCR](https://github.com/australiaitgroup/Guide-to-JS-Project/blob/master/VCR.md) for more information.
- See also [Response Validations](https://github.com/australiaitgroup/Guide-to-JS-Project/blob/master/api-design.md#response-validations)
